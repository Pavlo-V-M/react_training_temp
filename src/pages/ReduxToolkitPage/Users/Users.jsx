...
import { deleteUser } from "../../../redux/toolkit/users/users-actions";
import { setFilter } from "../../../redux/toolkit/filter/filter-actions";
// import { getUsersr } from "../../../redux/users/users-selectors";
import { getFilter } from "../../../redux/toolkit/filter/filter-selectors";
...
const Users = () => {
  ...
  <div className={styles.listWrap}>
      ...
      {Array.isArray(users) ? (
        users.map((user) => (
          <li key={user.id} className={styles.listItm}>
            <Link to={`/redux-toolkit/users/${user.id}`}>{user.name}</Link>
            <button className={styles.btn}
              // type="button" onClick={() => navigate(`/redux-toolkit/users/edit/${user.id}`)}
              type="button" onClick={() => navigate("/redux-toolkit/users/edit")}
            >Edit
            </button>
            <button className={styles.btn}
    ...
    <button className={styles.btn} type="button"
      onClick={() => navigate("/redux-toolkit/users/new")}
    >Create New</button>
  </div>);
};
